...
UIElement::UIElement(const std::string& tag, const Vector2f& pos, const float width, const float height, const bool hasLogic)
	: m_Tag{ tag }
	, m_Pos{ pos }
	, m_Width{ width }
	, m_Height{ height }
...
std::function<void()> UIElement::GetActionFunction(const std::string& key)
{
	const std::map<std::string, std::function<void()>>::const_iterator cit = m_FunctionMap.find(key);
